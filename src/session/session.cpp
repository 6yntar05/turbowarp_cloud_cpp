...
void session::handshake(std::optional<decltype(handlers::on_handshake)> handler) {
    do_handshake(decltype(handlers::on_handshake){
        .success =
            [handler](auto context) mutable { context.owner->do_accept(std::move(handler)); },
...
void session::read(handler<handlers::context, std::tuple<std::string>,
    ...
    do_read(decltype(handlers::on_read){
        .success = [handler](
                       ...
                       auto data) { handler.success(context, beast::buffers_to_string(data)); },
...
void session::send(const asio::const_buffer &data,
...
void session::do_handshake(std::optional<decltype(handlers::on_handshake)> handler) {
    ws_.next_layer().async_handshake(ssl::stream_base::server, [self = shared_from_this(), handler](
                                                                   boost::system::error_code ec) {
        handlers::context context{self};

        if (ec) {
            self->handlers_.on_handshake.failure(context, ec);
            if (handler.has_value()) handler->failure(context, ec);
            return;
        }

        self->handlers_.on_handshake.success(context);
        if (handler.has_value()) handler->success(context);
    });
}

void session::do_accept(std::optional<decltype(handlers::on_accept)> handler) {
    ws_.async_accept([self = shared_from_this(), handler](boost::system::error_code ec) {
        handlers::context context{self};
...
void session::do_read(std::optional<decltype(handlers::on_read)> handler) {
    ...
    ws_.async_read(buffer_, [self = shared_from_this(), handler = std::move(handler)](
        ...
        if (ec) {
            ...
            if (handler.has_value()) handler->failure(context, bytes_transferred, ec);
            return;
        }
...
void session::do_write(const asio::const_buffer &data,
    ...
    ws_.async_write(buffer_.data(),
                    [self = shared_from_this(), handler = std::move(handler)](
                        ...
                        if (handler.has_value()) handler->success(context, bytes_transferred);
                    });
}
